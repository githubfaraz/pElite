...
function BlogDetails({ params }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`https://pets-elite-backend-nine.vercel.app/api/blogs/${params.id}`);
        if (!res.ok) {
...
export async function metadata({ params }) {
  const res = await fetch(`https://pets-elite-backend-nine.vercel.app/api/blogs/${params._id}`);
